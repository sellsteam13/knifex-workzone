document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.querySelector('.game-mines') && document.querySelector('.game-mines-area__block')) {
        ...
        allBlocks.forEach(block => {
            ...
            let button = new ExplosiveButton(block);
        });
    }
...
class ExplosiveButton {
    constructor(el) {
        ...
        this.piecesY = 4;
        this.duration = 1000;
        this.explodingBlocks = document.querySelectorAll('.game-mines-area__block');
    ...
    explode(duration) {
        ...
        if (!this.element.classList.contains(explodingState)) {
            this.explodingBlocks.forEach(eachBlock => {
                eachBlock.classList.remove(explodingState);
            });
...
class Particle {
    ...
    runSequence(el, keyframesArray, duration = 1e3, easing = "linear", delay = 300) {
